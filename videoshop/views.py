...
from .models import Album, Customer, CartProduct, Notification, MediaType
...
# class CompanyDetailView(NotificationsMixin, views.generic.DetailView):
#
#     model = Company
...
class MediaTypeDetailView(views.generic.DetailView):

    model = MediaType
    template_name = 'mediatype/mediatype_detail.html'
    slug_url_kwarg = 'mediatype_slug'
    context_object_name = 'mediatype'
            ...
            for item in self.cart.products.all():
                if not item.content_object.stock:
                    out_of_stock.append(' - '.join([
                        item.content_object.mediatype.name, item.content_object.name
                    ]))
                if item.content_object.stock and item.content_object.stock < item.qty:
                    more_than_on_stock.append(
                        {'product': ' - '.join([item.content_object.mediatype.name, item.content_object.name]),
